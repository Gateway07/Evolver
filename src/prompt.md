...
This prompt defines the **Level L1** OpenEvolve loop for discovering **systematic NotFound counterexamples** in an address-search Java application.
Analyze application code to understand the search process and identify potential sources of errors/bugs and producing the following **artifacts** per iteration:
...
You MUST operate within the axioms, database schema, tracing workflow, application code and evaluator described below.
...
- A “counterexample generator” is always a **valid SQL WHERE-suffix** that selects a subset of rows from `osmand.address`.
...
You have the following key objects (not exhaustive): [@osmand_ddl.md](../agents/osmand_ddl.md)
...
- `runId, status, totalCount, failedCount, foundCount, partialFoundCount, totalDurationMs, totalBytes, searchDurationMs`

### Derived quantities
Your goal to maximize: 
1. Main objective: `notFoundRate = notFoundCount / max(totalCount, 1)` where `notFoundCount = totalCount - foundCount - failedCount - partialFoundCount`.
2. Cost proxies: `searchDurationMs / totalDurationMs`. Bigger `searchDurationMs` and `totalBytes` means performance problem and can be considered as a cost. 
...
## Tools (High-level usage only; SKILL.md will define exact constraints)

### psql.exe (Database inspection & feature computation)
Use `psql.exe` (through the provided skill wrapper) to:
- compute feature statistics 
- compute stratified samples and group membership
- read `osmand.run` and `osmand.run_result` to evaluate reproducibility, reasons, and coverage

**Do NOT** perform destructive operations.

### curl.exe (REST evaluator + tracing API)
Use `curl.exe` (through the provided skill wrapper) to:
- call `/admin/search-test/search-by-sql`
- manage tracing runs (`/tracing/...`)
- fetch trace logs

## Tracing API (Discovery mechanism via runtime variables)

Use tracing to observe runtime variables and validate/derive experiment and theory hypotheses without unit-tests. The tracing workflow and safety rules are fixed. [@tracing.md](../agents/skills/curl/tracing.md)

### Required tracing workflow
1) Generate `tracingId` and cleanup:
   - `DELETE http://localhost:8080/tracing/{tracingId}` 
2) Setup breakpoints:
   - `POST http://localhost:8080/tracing/{tracingId}/points`
   - `Content-Type: application/json`
   - Body: JSON array of breakpoint objects (you will fill breakpoint list as needed)
3) Execute one or multiple search-test requests with tracing enabled:
   - Only endpoints under `/admin/search-test/*` are traced 
   - MUST include `X-TRACING_MDC_KEY: {tracingId}` 
   - For batch: `/admin/search-test/search-by-sql` with WHERE-suffix body 
4) Fetch trace logs:
   - `GET http://localhost:8080/tracing/{tracingId}/logs` with `X-TRACING_MDC_KEY` 
   - Use trace events to extract runtime variables, intermediate scores, candidate list sizes, pruning thresholds, token normalization outputs, etc.
### Important: tracingId vs experiment runId (DB)
- `tracingId` is used ONLY for tracing API endpoints and attaching trace context to requests.
- `experiment runId` is `osmand.run.id` created by the evaluator run and stored in DB.
- L1 MUST store BOTH in artifacts:
  - `experiment.run_db_refs.run_id` (or a lookup plan if evaluator does not return it directly)
  - `tracing_evidence.breakpoints_json` (the exact JSON sent to `/points`)
  - `tracing_evidence.logs_json` (the exact JSON received from `/logs`)
...
To prevent shallow “explanations” that do not match real runtime behavior, every Theory MUST include a **surrogate model build** artifact that approximates (at least partially) the application’s index logic in a formal system.
...
The Mechanism/Theory output is only admissible if it meets ALL requirements below. These are not “nice to have”; they are acceptance gates.

### M1) Predictive generalization on HOLDOUT (required)
- The theory MUST achieve measurable predictive value on HOLDOUT data under a FIXED protocol (scoring-only holdout).
- Metrics to report (choose at least two; exact thresholds are placeholders): `{MECH_METRICS}` e.g. AUC, F1, top-k lift, calibration slope.
- HOLDOUT evaluation MUST NOT involve active group search or optimization. (HOLDOUT is scoring only.)

### M2) Explanatory compression / reuse (required)
...
### M4) Bounded interpretability (required)
...
Produce `canonical_json` with these rules:
1) UTF-8 encoding.
...
Compute `canonical_sql` with these rules:
1) Reject non-WHERE-suffix constructs:
   - No `;`, no multiple statements, no DDL/DML (`DROP`, `DELETE`, `UPDATE`, `INSERT`, `ALTER`, `CREATE`, `TRUNCATE`, `COPY`, `CALL`, `DO`).
   - No comments (`--`, `/*`, `*/`).
...
- `theory_signature = sha256(canonical_json(theory_dsl))` (or `canonical_sql` if theory is SQL-rule DSL)
...
- Each object MUST have deterministic definitions (no `now()`, no `random()`, no dependence on run tables for prediction).
...
- L4: Multi-stage mechanism with reason_family mapping aligned to tracing (predicts both NotFound risk and reason families).
...
3. If needed for Theory acceleration, create IMMUTABLE theory schema objects (views; optional materialized views) under the DB write policy and build budget above.
